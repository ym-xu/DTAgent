...
# === Mode and layout detection (slides vs doc; single vs double column) ===
...
def sample_page_images(source_dir: str, pages: List[int], *, k: int = 2, seed: Optional[int] = None) -> List[str]:
    """
    Choose up to k page images with a preference: include page 0 first (or the first page)
    and then sample remaining randomly. Returns existing or freshly rendered image paths.
    """
    if not pages:
        return []
    uniq_pages = sorted({int(p) for p in pages if isinstance(p, int)})
    ...
    if k >= 1:
        if 0 in uniq_pages:
            ...
            selected_pages.append(uniq_pages[0])
    if k > 1:
        remaining = [p for p in uniq_pages if p not in selected_pages]
        rng.shuffle(remaining)
        take = max(0, k - len(selected_pages))
        selected_pages.extend(remaining[:take])
...
def detect_mode_and_columns_via_gpt(
    source_dir: str,
    pages: List[int],
    *,
    ...
    """
    Use GPT-4o to decide document mode (doc or slides) and whether each physical page contains two logical pages side-by-side (two-up scan).
    Picks up to 3 page images (cover page 0 + two random pages). Returns (mode, columns, confidence) where columns=2 iff two-up else 1.
    On failure, returns ("doc", 1, 0.0).
    """
    try:
    ...
    imgs = sample_page_images(source_dir, pages, k=3)
    if not imgs:
        return "doc", 1, 0.0

    prompt = (
        "You are given 1-3 pages of a PDF as images.\n"
    ...
    try:
        raw = gpt_llm_call(messages, images=imgs, model=model, api_key=api_key, json_mode=True)
        # Print raw GPT-4o output for debugging/visibility
        try:
            print(f"[{model}]", raw)
        except Exception:
            pass
        obj = json.loads(raw)
        mode = obj.get("mode")
        # Prefer new field two_up; fallback to legacy columns==2
        two_up = obj.get("two_up")
        cols = obj.get("columns")
        conf = obj.get("confidence", 0.0)
        mode_str = "slides" if str(mode).lower().strip() == "slides" else "doc"
        two_up_bool = False
        if isinstance(two_up, bool):
            two_up_bool = two_up
        elif isinstance(two_up, str):
            two_up_bool = two_up.strip().lower() in ("true", "yes", "y", "1")
        elif isinstance(cols, int):
            two_up_bool = (cols == 2)
        else:
            try:
                two_up_bool = int(cols) == 2
            except Exception:
                two_up_bool = False
        cols_int = 2 if two_up_bool else 1
        try:
            conf_f = float(conf)
        except Exception:
            conf_f = 0.0
        return mode_str, cols_int, conf_f
    except Exception as e:
        try:
            print("[gpt-4o detect] failed:", e)
        except Exception:
            pass
        return "doc", 1, 0.0


## Page label detection utilities removed for redesign


## detect_page_labels removed for redesign


## postprocess_logical_pages removed for redesign


## apply_logical_pages_to_flat removed for redesign


## annotate_logical_pages removed for redesign


def detect_mode_and_columns(
    source_dir: str,
    pages: List[int],
) -> Tuple[str, int, float]:
    """
    Detect mode/columns using GPT-4o only. Returns (mode, columns, confidence).
    """
    return detect_mode_and_columns_via_gpt(source_dir, pages)


# === Logical page number (simple offset) detection ===
...
def iter_headings(flat_root: dict, *, include_media: bool = True):
    """Yield heading-like nodes (level>=1) from a flat doctree root.
    include_media=True 时，带上被提升为标题层级的 image/table。
    """
    children = list(flat_root.get("children", [])) if isinstance(flat_root, dict) else []
    for n in children:
